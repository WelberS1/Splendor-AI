...
class MidtermStrategy:
            ...
            for level in board_state.dealt:
                for cards in level:
                    ...
                    if cards.code not in available_cards.get(cards.colour):
                        total_cost = 0
                        k = 0
                        for colour, cost in cards.cost.items():
                            remain_cost = max(cost - len(state.cards.get(colour)) - state.gems.get(colour), 0)
                            if remain_cost != 0:
                                for gem_colour in action['collected_gems'].keys():
                                    if colour == gem_colour:
                                        k += 1
                            total_cost += remain_cost
                        if total_cost == 0:
                            IPR = 1
                        else:
                            IPR = k / total_cost
                    priority += IPR*card_priority
    ...
    def calculate_buy_values(self, start_state, agent_state, rival_state, board_state, action, get_successors):
        ...
        values.append(0)
        if len(board_state.nobles) != 0:
            for colour, number in agent_state.cards.items():
                if len(number) >= 4 and CARDS[action['card'].code][0] == colour:
                    values.pop()
                    values.append(1)
                break
        else:
    ...
    def q_learning(self, start_state, get_legal_actions, get_successors):
        ...
        weight_list = [[14.25239525231233, 9.152336300451607, -17.11478695463048],
                       [16.562437131461706, 4.009619997733457, 7.885213045369536, 4.23588280448085, 14.764117195519155,
